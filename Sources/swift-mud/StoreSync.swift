...
class StoreSync {
    ...
    private func handleLog(chainId: UInt64, log: EthereumLogObject) {
        do {
            switch log.topics[0] {
    ...
    private func getChainId() async throws -> UInt64 {
        ...
        guard let hex = chainIdValue?.hex(), let chainId = UInt64(hexString: hex) else { throw StoreSyncError.chainIdNotFound }
        return chainId
    }
}
